	...
	internal class LoadContext: BaseContext
	{
		struct SimplePropertyInfo
		{
			...
			public void SetValue(object obj, object value)
			{
				...
				} else if (AttachedProperty is not null && obj is BaseObject)
				{
		...
		public Dictionary<string, FSColor> Colors;
		...
		public void Load<T>(object obj, XElement el, T handler) where T : class
		{
			...
			ParseProperties(type, false, out var complexProperties, out var simpleProperties);

			string newName;
			foreach (var attr in el.Attributes())
			{
				var propertyName = attr.Name.ToString();
				if (LegacyPropertyNames is not null && LegacyPropertyNames.TryGetValue(propertyName, out newName))
				{
				...
				if (simplePropertyInfo is not null)
				{
					object value = null;
					...
					{
						if (Colors is not null && Colors.TryGetValue(attr.Value, out var color))
						{
							value = color;
						}
						else
				...
				else if (handler is not null && type.GetEvent(attr.Name.LocalName) is not null)
				{
				...
				else
				{
					...
					if (propertyName.StartsWith(UserDataAttributePrefix) && baseObject is not null)
					{
			...
			var contentProperty = (from p in complexProperties
								   ...
								   is not null select p).FirstOrDefault();

			foreach (var child in el.Elements())
			{
				var childName = child.Name.ToString();
				if (NodesToIgnore is not null && NodesToIgnore.Contains(childName))
				{
					continue;
				}
				...
				if (LegacyPropertyNames is not null && LegacyPropertyNames.TryGetValue(childName, out newName))
				{
					childName = newName;
				}
					...
					do
					{
						...
						if (asDict is not null)
						{
							...
							foreach (var child2 in child.Elements())
							{
								...
								var id = string.Empty;
								if (child2.Attribute(IdName) is not null)
								{
				...
				else
				{
					...
					if (LegacyClassNames is not null && LegacyClassNames.TryGetValue(widgetName, out newName))
					{
						widgetName = newName;
					}
