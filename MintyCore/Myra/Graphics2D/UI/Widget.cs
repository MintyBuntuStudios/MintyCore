...
using MintyCore.Myra.Graphics2D.UI.Properties;
...
namespace MintyCore.Myra.Graphics2D.UI
{
	...
	public partial class Widget : BaseObject, ITransformable
	{
		...
		public virtual Desktop? Desktop
		{
			...
			internal set
			{
				if (_desktop is not null && value is null)
				{
					...
					if (_desktop.Tooltip is not null && _desktop.Tooltip.Tag == this)
					{
		...
		public virtual IBrush GetCurrentBorder()
		{
			...
			else if (IsMouseInside && OverBorder is not null)
			{
				result = OverBorder;
			}
		...
		{
			var widgets = Parent is not null ? Parent.Children : Desktop.Widgets;
		...
		public void Render(RenderContext context)
		{
			...
			if (!string.IsNullOrEmpty(Tooltip) && (Desktop.Tooltip is null || Desktop.Tooltip.Tag != this) &&
				_lastMouseMovement is not null && (DateTime.Now - _lastMouseMovement.Value).TotalMilliseconds > MyraEnvironment.TooltipDelayInMs)
			{
			...
			if (oldScissorRectangle is not null)
			{
				// Restore scissor
			...
			{
				context.DrawRectangle(Bounds, FSColor.LightGreen);
			}

			if (MyraEnvironment.DrawKeyboardFocusedWidgetFrame && IsKeyboardFocused)
			{
				context.DrawRectangle(Bounds, FSColor.Red);
			}

			if (MyraEnvironment.DrawMouseHoveredWidgetFrame && IsMouseInside)
			{
				context.DrawRectangle(Bounds, FSColor.Yellow);
			}
		...
		public void UpdateArrange()
		{
			...
			if (HorizontalAlignment == HorizontalAlignment.Stretch && Width is not null && Width.Value < containerSize.X)
			{
				containerSize.X = Width.Value;
			}

			if (VerticalAlignment == VerticalAlignment.Stretch && Height is not null && Height.Value < containerSize.Y)
			{
		...
		{
			if (_startPos is null || !IsDraggable || Desktop is null)
			{
				return;
			}
			...
			var parentBounds = Parent is not null ? Parent.Bounds : Desktop.InternalBounds;
			if (newLeft < 0)
			{
				newLeft = 0;
			}
		...
		public Vector2 ToLocal(Vector2 source)
		{
			...
			{
				Matrix.Invert(Transform.Matrix, out var inverse);
				_inverseMatrix = inverse;
				_inverseMatrixDirty = false;
			}

			return Mathematics.Transform(source, ref _inverseMatrix);
		}
		...
		{
			if (Desktop is null || !Visible || !ContainsGlobalPoint(p))
			{
				return null;
			}
			...
			if (result is null && !InputFallsThrough(localPos))
			{
				result = this;
			}
