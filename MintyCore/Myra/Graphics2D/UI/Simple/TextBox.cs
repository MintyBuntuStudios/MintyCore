...
using MintyCore.Myra.Graphics2D.UI.TextEdit;
using MintyCore.Myra.Platform;
using MintyCore.Myra.TextCopy;
...
namespace MintyCore.Myra.Graphics2D.UI.Simple
{
	public class TextBox : Widget
	{
		...
		private int CursorWidth => 1 + (Cursor is not null ? Cursor.Size.X : 0);
		...
		private Point GetRenderPositionByIndex(int index)
		{
			...
			if (Text is not null)
			{
				if (index < Text.Length)
				{
					...
					if (glyphRender is not null)
					{
						x += glyphRender.Value.Bounds.Left;
						y += glyphRender.Value.LineTop;
					}
				}
				else if (_richTextLayout.Lines is not null && _richTextLayout.Lines.Count > 0)
				{
					// After last glyph
		...
		private void RenderSelection(RenderContext context)
		{
			...
			if (string.IsNullOrEmpty(Text) || Selection is null)
			{
				return;
			}
		...
		{
			if (_richTextLayout.Font is null)
			{
				return;
			}
			...
			else if (!Enabled && DisabledTextColor is not null)
			{
				textColor = DisabledTextColor.Value;
			}
			else if (IsKeyboardFocused && FocusedTextColor is not null)
			{
				...
				foreach (var line in _richTextLayout.Lines)
				{
					foreach (TextChunk chunk in line.Chunks)
					{
						foreach (var glyph in chunk.Glyphs)
						{
							...
							context.DrawRectangle(glyphBounds, FSColor.White);
						}
					}
				}
			}
			...
			if (Enabled && _cursorOn && Cursor is not null)
			{
