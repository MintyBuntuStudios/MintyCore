...
namespace MintyCore.Myra.Graphics2D.UI.Simple
{
	...
	public class Image : Widget, IPressable
	{
		...
		public FSColor Color { get; set; } = FSColor.White;
		...
		{
			var result = _image is not null ? _image.Size : Mathematics.PointZero;

			var overSize = _overImage is not null ? _overImage.Size : Mathematics.PointZero;
			...
			var pressedSize = _pressedImage is not null ? _pressedImage.Size : Mathematics.PointZero;
		...
		public override void InternalRender(RenderContext context)
		{
			...
			if (IsMouseInside && OverRenderable is not null)
			{
				image = OverRenderable;
			}

			if (IsPressed && PressedRenderable is not null)
			{
