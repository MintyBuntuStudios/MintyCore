...
namespace MintyCore.Myra.Graphics2D.UI.Simple
{
	public class Label : Widget
	{
		...
		[Category("Appearance")]
		public FSColor? DisabledTextColor
		{
			get; set;
		}
		...
		internal FSColor? PressedTextColor
		{
			get; set;
		}
		...
		public override void InternalRender(RenderContext context)
		{
			...
			if (!Enabled && DisabledTextColor is not null)
			{
				color = DisabledTextColor.Value;
				useChunkColor = false;
			}
			else if (IsPressed && PressedTextColor is not null)
			{
				color = PressedTextColor.Value;
				useChunkColor = false;
			}
			else if (IsMouseInside && OverTextColor is not null)
			{
			...
			catch (Exception ex)
			{
				...
				context.DrawRichText(_errorText, new Vector2(x, bounds.Y), FSColor.Red);
			}
		}
