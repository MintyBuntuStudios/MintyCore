...
namespace MintyCore.Myra.Graphics2D.UI.ColorPicker
{
    public partial class ColorPickerPanel
    {
        ...
        public static readonly FSColor[] UserColors = new[]
        {
            new FSColor(255, 255, 255),
            new FSColor(217, 217, 217),
            new FSColor(178, 178, 178),
            new FSColor(140, 140, 140),
            new FSColor(102, 102, 102),
            new FSColor(64, 64, 64),
            new FSColor(32, 32, 32),
            new FSColor(0, 0, 0),
            new FSColor(254, 57, 48),
            new FSColor(255, 149, 3),
            new FSColor(255, 204, 1),
            new FSColor(75, 217, 97),
            new FSColor(91, 198, 250),
            new FSColor(3, 121, 255),
            new FSColor(87, 86, 213),
            new FSColor(207, 86, 191)
        };
        ...
        {
            get => Color.R;
            set => Color = new FSColor(value, Color.G, Color.B, Color.A);
        }

        public byte G
        {
            get => Color.G;
            set => Color = new FSColor(Color.R, value, Color.B, Color.A);
        }

        public byte B
        {
            get => Color.B;
            set => Color = new FSColor(Color.R, Color.G, value, Color.A);
        }
        ...
        private int? SelectedUserColorIndex
        {
            get
            {
                if (_userColors.SelectedColumnIndex is null || _userColors.SelectedRowIndex is null)
                {
                    return null;
                }
        ...
        private void SetUserColor(int index, FSColor color)
        {
        ...
        {
            _saveColor.Enabled = SelectedUserColorIndex is not null;
        }
        ...
        private void RgbInputChanged(object? sender, EventArgs e)
        {
            ...
            if (byte.TryParse(st[0], out byte r) && byte.TryParse(st[1], out byte g) &&
                byte.TryParse(st[2], out byte b))
            {
                _inputRGB.Tag = true;
                OnColorChanged(new FSColor(r, g, b));
                _inputRGB.Tag = false;
            }
        }
        ...
        private void HexInputChanged(object? sender, EventArgs e)
        {
            ...
            if (color is not null)
            {
                _inputHEX.Tag = true;
                var c = color.Value;
                OnColorChanged(new FSColor(c.R, c.G, c.B));
                _inputHEX.Tag = false;
            }
        }
        ...
        private void OnColorChanged(FSColor c)
        {
            A = c.A / 255f;
        ...
        {
            var c = h.ToRGB();
            c = new FSColor(c.R, c.G, c.B, (byte)255);
            OnColorChanged(c, h);
        }

        private void OnColorChanged(FSColor rgb, ColorHSV hsv)
        {
            ...
            _colorWheel.Color = new FSColor((byte)(hsv.V * 255.0f / 100f), (byte)(hsv.V * 255.0f / 100f),
                (byte)(hsv.V * 255.0f / 100f));
