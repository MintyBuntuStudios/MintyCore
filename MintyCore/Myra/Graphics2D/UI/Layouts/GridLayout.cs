...
namespace MintyCore.Myra.Graphics2D.UI.Layouts
{
	public class GridLayout: ILayout
	{
		...
		{
			if (ColWidths is null || index < 0 || index >= ColWidths.Count)
			{
				return 0;
			}
		...
		{
			if (RowHeights is null || index < 0 || index >= RowHeights.Count)
			{
				return 0;
			}
		...
		public Point Measure(IEnumerable<Widget> widgets, Point availableSize)
		{
			...
			if (_widgetsByGridPosition is null ||
			...
			for (var row = 0; row < rows; ++row)
			{
				...
				{
					if (_widgetsByGridPosition[row, col] is null)
					{
