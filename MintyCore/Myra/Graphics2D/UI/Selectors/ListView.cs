...
namespace MintyCore.Myra.Graphics2D.UI.Selectors
{
	public class ListView : Widget
	{
		...
		private class WidgetsCollection : IList<Widget>, IList
		{
			...
			public bool Contains(object value) => Find((Widget)value) is not null;
			...
			public bool Contains(Widget item) => Find(item) is not null;
			...
			private Widget Find(Widget item)
			{
				foreach (var w in Container.Widgets)
				{
					...
					var asButton = w as ListViewButton;
					if (asButton is not null && asButton.Content == item)
					{
						return w;
					}
				}
		...
		public int? SelectedIndex
		{
			...
			set
			{
				if (value is null || value.Value < 0 || value.Value >= Widgets.Count)
				{
					SelectedItem = null;
					return;
				}
