...
namespace MintyCore.Myra.Graphics2D.UI.Selectors
{
	...
	public class ListBox : Selector<ScrollViewer, ListItem>
	{
		...
		protected override void OnSelectedItemChanged()
		{
			...
			if (_parentComboBox is not null)
			{
		...
		public override void OnKeyDown(Keys k)
		{
			...
			{
				case Keys.Up:
					if (SelectedIndex is null && Items.Count > 0)
					{
						SelectedIndex = Items.Count - 1;
						UpdateScrolling();
					}

					if (SelectedIndex is not null && SelectedIndex.Value > 0)
					{
				...
				case Keys.Down:
					...
					if (SelectedIndex is not null && SelectedIndex.Value < Items.Count - 1)
					{
						SelectedIndex = SelectedIndex.Value + 1;
						UpdateScrolling();
					}
