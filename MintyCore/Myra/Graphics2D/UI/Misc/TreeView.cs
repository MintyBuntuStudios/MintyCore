	...
	public class TreeView : Widget
	{
				...
				case Keys.Up:
					{
						...
						{
							if (index == 0 && SelectedRow.ParentNode is not null)
							{
		...
		public override void InternalRender(RenderContext context)
		{
			...
			if (SelectionBackground is not null)
			{
				...
				if (SelectedRow is not null && SelectedRow.RowVisible)
				{
			...
			{
				if (HoverRow is not null && SelectionHoverBackground is not null)
				{
					var rect = BuildRowRect(HoverRow);
					SelectionHoverBackground.Draw(context, rect);
				}
