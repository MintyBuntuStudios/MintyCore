...
namespace MintyCore.Myra.Graphics2D.UI
{
	public partial class Desktop : ITransformable, IDisposable
	{
		...
		public bool IsTouchOverGUI
		{
			get
			{
				return TouchPosition is not null && IsPointOverGUI(TouchPosition.Value);
			}
		}
		...
		private bool IsMenuBarActive
		{
			get
			{
				return MenuBar is not null && (MenuBar.OpenMenuItem is not null || IsAltDown);
			}
		}
		...
		public Desktop()
		{
			...
			if (Stylesheet.Current?.DesktopStyle is not null)
			{
		...
		{
			if (ContextMenu is not null && !ContextMenu.IsTouchInside)
			{
		...
		public void Render()
		{
			...
			if (_inputContext.MouseWheelWidget is not null)
			{
		...
		private void FocusNextWidget()
		{
			...
			if (focusChanged || FocusedKeyboardWidget is null)
			{
		...
		private static bool CanFocusWidget(Widget widget) =>
			widget is not null && widget.Visible &&
