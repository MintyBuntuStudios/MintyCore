...
namespace MintyCore.Myra.Graphics2D.UI.Containers
{
	...
	public class Grid : Container
	{
		...
		[Category("Debug")]
		[DefaultValue("White")]
		public FSColor GridLinesColor { get; set; }
		...
		private void RenderSelection(RenderContext context)
		{
			...
			switch (GridSelectionMode)
			{
				...
				case GridSelectionMode.Row:
					{
						if (HoverRowIndex is not null && HoverRowIndex != SelectedRowIndex && SelectionHoverBackground is not null)
						{
						...
						if (SelectedRowIndex is not null && SelectionBackground is not null)
						{
							var rect = new Rectangle(bounds.Left,
				...
				case GridSelectionMode.Column:
					{
						if (HoverColumnIndex is not null && HoverColumnIndex != SelectedColumnIndex && SelectionHoverBackground is not null)
						{
						...
						if (SelectedColumnIndex is not null && SelectionBackground is not null)
						{
				...
				case GridSelectionMode.Cell:
					{
						if (HoverRowIndex is not null && HoverColumnIndex is not null &&
							(HoverRowIndex != SelectedRowIndex || HoverColumnIndex != SelectedColumnIndex) &&
							SelectionHoverBackground is not null)
						{
							var rect = new Rectangle(CellLocationsX[HoverColumnIndex.Value] + bounds.Left - ColumnSpacing / 2,
								CellLocationsY[HoverRowIndex.Value] + bounds.Top - RowSpacing / 2,
								ColWidths[HoverColumnIndex.Value] + ColumnSpacing,
						...
						if (SelectedRowIndex is not null && SelectedColumnIndex is not null && SelectionBackground is not null)
						{
							var rect = new Rectangle(CellLocationsX[SelectedColumnIndex.Value] + bounds.Left - ColumnSpacing / 2,
								CellLocationsY[SelectedRowIndex.Value] + bounds.Top - RowSpacing / 2,
								ColWidths[SelectedColumnIndex.Value] + ColumnSpacing,
		...
		public override void OnTouchDown()
		{
			...
			if (HoverColumnIndex is not null)
			{
