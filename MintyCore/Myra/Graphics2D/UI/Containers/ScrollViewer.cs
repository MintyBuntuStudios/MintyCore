...
namespace MintyCore.Myra.Graphics2D.UI.Containers
{
	public class ScrollViewer : ContentControl
	{
		...
		private int HorizontalScrollbarHeight
		{
			...
			{
				var result = 0;
				if (HorizontalScrollBackground is not null)
				{
					result = HorizontalScrollBackground.Size.Y;
				}

				if (HorizontalScrollKnob is not null && HorizontalScrollKnob.Size.Y > result)
				{
		...
		private int VerticalScrollbarWidth
		{
			...
			{
				var result = 0;
				if (VerticalScrollBackground is not null)
				{
					result = VerticalScrollBackground.Size.X;
				}

				if (VerticalScrollKnob is not null && VerticalScrollKnob.Size.X > result)
				{
		...
		{
			if (!_startBoundsPos.HasValue || Desktop is null)
				return;
