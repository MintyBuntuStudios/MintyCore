	...
	public class SpinButton : Widget
	{
		...
		public float? Value
		{
			get
			{
				...
				if (float.TryParse(_textField.Text, out var result))
				{
					return result;
				}
			...
			set
			{
				...
				if (_textField.Text is not null)
				{
		...
		private void _textField_ValueChanging(object sender, ValueChangingEventArgs<string> e)
		{
			...
			else if (s == "-")
			{
				...
				if (Minimum is not null && Minimum.Value >= 0)
				{
					e.Cancel = true;
				}
			}
			else
			{
				...
				if (Integer)
				{
					...
					{
						if ((Minimum is not null && i < (int)Minimum) ||
							(Maximum is not null && i > (int)Maximum))
						{
							e.Cancel = true;
						}
						else
				...
				else
				{
					...
					{
						if ((Minimum is not null && f < Minimum) ||
							(Maximum is not null && f > Maximum))
						{
							e.Cancel = true;
						}
						else
		...
		public void ApplySpinButtonStyle(SpinButtonStyle style)
		{
			...
			if (style.TextBoxStyle is not null)
			{
				_textField.ApplyTextBoxStyle(style.TextBoxStyle);
			}

			if (style.UpButtonStyle is not null)
			{
				_upButton.ApplyImageButtonStyle(style.UpButtonStyle);
			}

			if (style.DownButtonStyle is not null)
			{
		...
		{
			if (!float.TryParse(_textField.Text, out var value))
			{
				value = 0;
			}
			value += _increment;
		...
		public override void OnLostKeyboardFocus()
		{
			...
			if (string.IsNullOrEmpty(_textField.Text) && !Nullable)
			{
				...
				else if (Maximum is not null && Maximum.Value < 0)
				{
