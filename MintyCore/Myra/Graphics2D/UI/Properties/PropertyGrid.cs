...
namespace MintyCore.Myra.Graphics2D.UI.Properties
{
	public class PropertyGrid : Widget
	{
		...
		private class SubGrid : Widget
		{
			...
			public SubGrid(PropertyGrid parent, object value, string header, string category, string filter, Record parentProperty)
			{
				...
				var expanded = true;
				if (parentProperty is not null && parentProperty.FindAttribute<DesignerFoldedAttribute>() is not null)
				{
					expanded = false;
				}
			...
			{
				if (_propertyGrid.PropertyGridStyle.SelectionHoverBackground is not null && IsMouseInside)
				{
		...
		{
			if (CustomSetter is not null && CustomSetter(record, obj, value))
			{
				return;
			}
		...
		private ComboView CreateCustomValuesEditor(Record record, object[] customValues, bool hasSetter)
		{
			...
			if (hasSetter)
			{
				...
				{
					var item = cv.SelectedIndex is not null ? customValues[cv.SelectedIndex.Value] : null;
		...
		private SpinButton CreateNumericEditor(Record record, bool hasSetter)
		{
			...
			var spinButton = new SpinButton
			{
				...
				Value = value is not null ? (float)Convert.ChangeType(value, typeof(float)) : default(float?)
			};
			...
			if (hasSetter)
			{
				spinButton.ValueChanged += (sender, args) =>
				{
					try
					{
						...
						if (spinButton.Value is not null)
						{
						...
						if (record.Type.IsValueType)
						{
							...
							while (pg is not null && tg._parentProperty is not null && tg._parentProperty.Type.IsValueType)
							{
				...
				tf.TextChanged += (sender, args) =>
				{
					try
					{
						...
						if (record.Type.IsValueType)
						{
							var tg = this;
							var pg = tg._parentGrid;
							while (pg is not null && tg._parentProperty is not null)
							{
								tg._parentProperty.SetValue(pg._object, tg._object);
		...
		{
			if (Settings.AssetManager is null)
			{
				return null;
			}
			...
			else if (Settings.ImagePropertyValueGetter is not null)
			{
				...
				button.Click += (sender, args) =>
				{
					...
					dlg.Closed += (s, a) =>
					{
						...
						try
						{
							...
							else if (Settings.ImagePropertyValueSetter is not null)
							{
		...
		private void FillSubGrid(ref int y, IReadOnlyList<Record> records)
		{
			for (var i = 0; i < records.Count; ++i)
			{
				...
				if (_parentProperty is not null && _parentProperty.Type.IsValueType && !_parentProperty.HasSetter)
				{
					hasSetter = false;
				}
				...
				if ((valueWidget = CustomWidgetProvider?.Invoke(record, _object)) is not null)
				{

				}
				else if (CustomValuesProvider is not null && (customValues = CustomValuesProvider(record)) is not null)
				{
				...
				else if (propertyType == typeof(FSColor) || propertyType == typeof(FSColor?))
				{
				...
				else if (propertyType == typeof(string) && record.FindAttribute<FilePathAttribute>() is not null)
				{
		...
		public void Rebuild()
		{
			...
			foreach (var property in properties)
			{
				...
				if (readOnlyAttr is not null && readOnlyAttr.IsReadOnly)
				{
					hasSetter = false;
				}
				...
				record.Category = categoryAttr is not null ? categoryAttr.Category : DefaultCategoryName;
			...
			foreach (var field in fields)
			{
				...
				{
					HasSetter = hasSetter,
					Category = categoryAttr is not null ? categoryAttr.Category : DefaultCategoryName
				};
			...
			if (asWidget is not null && ParentType is not null)
			{
			...
			for (var i = 0; i < records.Count; ++i)
			{
				...
				if (!_records.TryGetValue(record.Category, out List<Record> categoryRecords))
				{
			...
			var y = 0;
			if (_records.TryGetValue(Category, out List<Record> defaultCategoryRecords))
			{
