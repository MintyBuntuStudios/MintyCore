...
namespace MintyCore.Myra.Graphics2D.TextureAtlases
{
	public static class Gdx
	{
		...
		{
			public required FontTextureWrapper Texture { get; set; }
		}

		private class GDXSpriteData
		{
			public required GDXPageData PageData { get; set; }
		...
		public static TextureRegionAtlas FromGDX(string data, Func<string, FontTextureWrapper> textureLoader)
		{
				...
				while (true)
				{
					...
					switch (key)
					{
						...
						case "repeat":
							break;
						case "rotate":
							bool.TryParse(value, out var isRotated);
