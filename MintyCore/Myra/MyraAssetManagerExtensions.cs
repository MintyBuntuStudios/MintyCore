...
namespace MintyCore.Myra
{
	public static class MyraAssetManagerExtensions
	{
		internal class Texture2DWrapper
		{
			...
			public Texture2DWrapper(int width, int height, FontTextureWrapper texture)
			{
				Width = width;
				Height = height;
				Texture = texture;
		...
		{
			public FontTextureWrapper? ExistingTexture { get; set; }
		...
		private static AssetLoader<FontSystem> _fontSystemLoader = (manager, assetName, settings, tag) =>
		{
			...
			if (fontSystemLoadingSettings is not null)
			{
			...
			if (fontSystemLoadingSettings is not null && fontSystemLoadingSettings.AdditionalFonts is not null)
			{
		...
		private static AssetLoader<Stylesheet> _stylesheetLoader = (manager, assetName, settings, tag) =>
		{
			...
			FontTextureWrapper existingTexture = null;
			var existingTextureUsedSpace = Rectangle.Empty;
			if (usedSpaceAttr is not null)
			{
		...
		public static FontSystem LoadFontSystem(this AssetManager assetManager, string assetName, string[]? additionalFonts = null, FontTextureWrapper? existingTexture = null, Rectangle existingTextureUsedSpace = default(Rectangle))
		{
			FontSystemLoadingSettings? settings = null;
			if (additionalFonts is not null || existingTexture is not null)
			{
