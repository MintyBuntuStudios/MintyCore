	...
	internal class LayoutBuilder
	{
		...
		private bool ProcessCommand(ref int i, ref ChunkInfo r, out bool chunkFilled)
		{
			...
			else
			{
				...
				switch (command)
				{
					...
					case 'f':
						if (_richTextSettings.FontResolver is null)
						{
						...
						break;
					case 'i':
						if (_richTextSettings.ImageResolver is null)
						{
		...
		private ChunkInfo GetNextChunk(ref int i, int? remainingWidth)
		{
			...
			for (; i < _text.Length; ++i, ++r.EndIndex)
			{
				...
				if (remainingWidth is not null && sz.X > remainingWidth.Value && i > r.StartIndex &&
					(lastBreakMeasure is not null || _currentLineChunks == 0))
				{
					if (lastBreakMeasure is not null)
					{
		...
		public Point Layout(string text, SpriteFontBase font, int? rowWidth, int? height, bool measureRun = false)
		{
			...
			while (i < _text.Length)
			{
				...
				if (_width is not null && c.Width > _width.Value && _currentLineChunks > 0)
				{
			...
			// Apply height
			if (!_measureRun && rowWidth is not null && _height is not null && size.Y > _height.Value)
			{
					...
					lines[lines.Count - 1].Chunks.Count > 0)
				{
					...
					if (lastChunk is not null && !string.IsNullOrEmpty(lastChunk.Text))
					{
